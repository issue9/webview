...
void script_message_received(WebKitUserContentManager* m, WebKitJavascriptResult* rslt, gpointer user_data) {
...
void dispatch() {
    g_idle_add_full(G_PRIORITY_HIGH_IDLE, dispatch_cb, NULL, NULL);
}
...
App* create_gtk(bool debug) {
    ...
    WebKitUserContentManager* m = userContentManager(wv);
    g_signal_connect(m, "script-message-received::external", G_CALLBACK(script_message_received), NULL);
...
void move(App* app, int x, int y) {
    gtk_window_move(GTK_WINDOW(app->win), x, y);
}
